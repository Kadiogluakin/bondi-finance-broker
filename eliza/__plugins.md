# 🧩 Plugins
Source: https://elizaos.github.io/eliza/docs/packages/plugins/

📦 PackagesPlugin SystemOn this page🧩 Plugins
Overview​
...
Wallet Generation on First Run
...
REMOTE_ATTESTATION - Generate a Remote Attestation Quote based on runtime.agentId when the agent is prompted for a remote attestation. The quote is uploaded to the proof.t16z.com service and the agent is informed of the attestation report URL.
...
TRANSFER_FUEL_ETH - Transfer ETH to a given Fuel address. - Inputs: - toAddress (string): The Fuel address to transfer ETH to. - amount (string): The amount of ETH to transfer. - Outputs: Confirmation message with transaction details. - Example:
...
Makes Eliza TEE-aware by using the Marlin Oyster platform tooling with the goal of making Eliza agents verifiable and private.
Configuration:
...
For local development and testing, you can use a mock attestation server that generates attestations based on a local root of trust. See the linked README for more detailed information.
From source:
...
The Allora Network plugin seamlessly empowers Eliza agents with real-time, advanced, self-improving AI inferences, delivering high-performance insights without introducing any additional complexity.
Setup and Configuration​
...
ALLORA_API_KEY: Create an API key by creating an account.
...
BUY_CURVES_TOKEN - Buy curves tokens for a subject address
...
SELL_CURVES_TOKEN - Sell curves tokens for a subject address
...
WITHDRAW_CURVES_TOKEN - Convert curves tokens to their ERC20 equivalent
...
DEPOSIT_CURVES_TOKEN - Convert ERC20 tokens back to curves tokens
...
MINT_CURVES_ERC20 - Mint new ERC20 token for curves holdings
...
GET_CURVES_BALANCE - Check curves token balance
...
GET_CURVES_BUY_PRICE - Get price quote for buying curves
...
GET_CURVES_SELL_PRICE - Get price quote for selling curves
...
GET_CURVES_ERC20_DETAILS - Get curves token respective ERC20 details
...
For detailed API documentation and examples, see the API Reference.Edit this pageLast updated on Jan 25, 2025 by Marouane Trabelsi